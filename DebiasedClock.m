function varargout = DebiasedClock(varargin)
% DEBIASEDCLOCK MATLAB code for DebiasedClock.fig
%      DEBIASEDCLOCK, by itself, creates a new DEBIASEDCLOCK or raises the existing
%      singleton*.
%
%      H = DEBIASEDCLOCK returns the handle to a new DEBIASEDCLOCK or the handle to
%      the existing singleton*.
%
%      DEBIASEDCLOCK('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DEBIASEDCLOCK.M with the given input arguments.
%
%      DEBIASEDCLOCK('Property','Value',...) creates a new DEBIASEDCLOCK or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DebiasedClock_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DebiasedClock_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DebiasedClock

% Last Modified by GUIDE v2.5 13-Jan-2024 15:34:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @DebiasedClock_OpeningFcn, ...
    'gui_OutputFcn',  @DebiasedClock_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before DebiasedClock is made visible.
function DebiasedClock_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DebiasedClock (see VARARGIN)
% Choose default command line output for DebiasedClock
...
function varargout = DebiasedClock_OutputFcn(hObject, eventdata, handles)
...
elseif index==4 %print figure
    if ~isfield(handles,'data')
        d=dialog('Position',[300 300 500 150],'Name','print figure');
        uicontrol('Parent',d,...
               'Style','text',...
               'Position',[20 80 410 40],...
               'String','No data are plotted');
        return;
    end
    data=handles.data;%???
    if 0
